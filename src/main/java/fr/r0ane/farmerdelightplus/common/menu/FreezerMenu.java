...
import fr.r0ane.farmerdelightplus.AllGui;
...
public class FreezerMenu extends AbstractContainerMenu {
    ...
    public FreezerMenu(int pContainerId, Inventory inv, BlockEntity entity, ContainerData data) {
        super(AllGui.FREEZER_MENU.get(), pContainerId);
        ...
        //data.set(2, data.get(2) * 1000 + data.get(3) );

        addDataSlots(data);
        //setData(2, data.get(2) * 1000 + data.get(3));
    }
    ...
    public int getIceScaledProgress() {
        if (itIsIntoAnIceBiome()) return 0;

        int progress = (int) Math.floor(this.data.get(2) / 100000);
        int maxProgress = this.data.get(2) % 100000; // Max Progress
        int progressArrowSize = 13; // This is the height in pixels of your arrow

        progress = maxProgress - progress;

        return maxProgress != 0 && progress != 0 ? progress * progressArrowSize / maxProgress : 13;
    }

    public boolean itIsIntoAnIceBiome() {
        return this.level.getBiome(this.blockEntity.getBlockPos()).is(Tags.Biomes.IS_COLD);
    }
